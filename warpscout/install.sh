...
say ""
say "Recommended filtered-network workflow:"
say "  cd ${STATE_DIR}"
say "  JT=4   # safe starting point for the 512 MB target"
say "  warpscout scan -p awg -P -jt \"\$JT\" -gen-i1 quic"
say "  warpscout scan -p awg -P -jt \"\$JT\" -gen-i1 quic -exclude-country RU"
say "  warpscout find-junk -jt \"\$JT\" -gen-i1 random   # only if I1 profiles fail"
say "  warpscout find-sni -p masque-h2 -jt \"\$JT\""
say "  warpscout find-sni -p masque -jt \"\$JT\""
say ""
say "Use the SNI printed by find-sni for the matching H2/H3 scan."
say "MASQUE node/country filters are intentionally unsupported: its NODE is path-selected."
say "Plain WG is useful only as a baseline on filtering networks."
say "On a tested KN-1812 with 1 GB RAM, -jt 16 ran comfortably."
